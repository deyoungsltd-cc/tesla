...
export default function Admin() {
  ...
  const [editWalletType, setEditWalletType] = useState('live');
  ...
  const fetchAuditLog = useCallback(async (page: number) => {
    ...
    try {
      const res = await api.admin.auditLog(`page=${page}&limit=${AUDIT_PAGE_SIZE}`);
      const data = res.logs || res.data || res;
      if (Array.isArray(data)) {
        setAuditLogs(data);
        setAuditTotal(data.length);
      } else {
        setAuditLogs(data.logs || data.items || data.data || []);
  ...
  useEffect(() => {
    ...
    fetchKyc();
    fetchAuditLog(1);
  }, [isAdmin, fetchStats, fetchUsers, fetchDeposits, fetchWithdrawals, fetchKyc, fetchAuditLog]);
  ...
  const handleAdjustBalance = async () => {
    if (!editBalanceUserId || !editAmount) return;
    ...
    try {
      await api.admin.patchUser({
        userId: editBalanceUserId,
        action: 'adjust_balance',
        walletType: editWalletType,
        amount,
      });
      toast.success(`Balance adjusted by ${amount > 0 ? '+' : ''}$${amount.toFixed(2)} on ${editWalletType} wallet`);
      setEditAmount('');
      setEditBalanceUserId(null);
      // Refresh user detail if dialog is open
      if (userDetailOpen && userDetail?.id === editBalanceUserId) {
        const res = await api.admin.userDetail(editBalanceUserId);
        setUserDetail(res.user || res.data || res);
      }
      fetchUsers(userPage);
      fetchStats();
        ...
        <TabsContent value="messages">
          <Card className="border-white/[0.04] bg-white/[0.01]">
            ...
            <CardContent className="space-y-4">
              {/* Recipient Selection */}
              <div className="space-y-2">
                <Label className="text-xs text-neutral-400">Recipients</Label>
                <div className="flex gap-3">
                  <button
                    onClick={() => setMsgBroadcast(true)}
                    className={`flex-1 py-2.5 px-4 rounded-lg text-xs font-medium transition-all border ${
                      msgBroadcast
                        ? 'bg-red-500/10 border-red-500/30 text-red-400'
                        : 'bg-white/[0.02] border-white/[0.06] text-neutral-500 hover:border-white/[0.12]'
                    }`}
                  >
                    All Users
                  </button>
                  <button
                    onClick={() => setMsgBroadcast(false)}
                    className={`flex-1 py-2.5 px-4 rounded-lg text-xs font-medium transition-all border ${
                      !msgBroadcast
                        ? 'bg-red-500/10 border-red-500/30 text-red-400'
                        : 'bg-white/[0.02] border-white/[0.06] text-neutral-500 hover:border-white/[0.12]'
              ...
              {/* User picker (when not broadcasting) */}
              {!msgBroadcast && (
                <div className="space-y-2">
                  <Label className="text-xs text-neutral-400">Select Users</Label>
                  <div className="max-h-40 overflow-y-auto border border-white/[0.06] rounded-lg bg-white/[0.01]">
                    {usersLoading ? (
                      <div className="p-4 text-center text-neutral-500 text-xs">Loading users...</div>
                    ) : users.length === 0 ? (
                      <div className="p-4 text-center text-neutral-500 text-xs">No users found</div>
                    ) : (
                      users.map(u => (
                        <label
                          key={u.id}
                          className="flex items-center gap-2 px-3 py-2 hover:bg-white/[0.02] cursor-pointer border-b border-white/[0.03] last:border-0"
                        >
                          <input
                            type="checkbox"
                            ...
                            className="rounded border-white/[0.15] bg-white/[0.03] text-red-500 focus:ring-red-500/30"
                          />
                          <span className="text-xs text-neutral-300">{u.email}</span>
                        </label>
                      ))
                    )}
                  </div>
                  <p className="text-[10px] text-neutral-600">{msgSelectedUsers.length} user(s) selected</p>
                </div>
              )}

              {/* Message Type */}
              <div className="space-y-2">
                <Label className="text-xs text-neutral-400">Message Type</Label>
                <Select value={msgType} onValueChange={setMsgType}>
                  <SelectTrigger className="bg-[#0a0a0a] border-white/[0.06] text-white text-xs">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent className="bg-[#0a0a0a] border-white/[0.06]">
                    <SelectItem value="custom" className="text-neutral-300 text-xs">Custom Message</SelectItem>
                    <SelectItem value="billing" className="text-neutral-300 text-xs">Billing Notice</SelectItem>
                    <SelectItem value="announcement" className="text-neutral-300 text-xs">Announcement</SelectItem>
                  </SelectContent>
                </Select>
              </div>
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  className="bg-[#0a0a0a] border-white/[0.06] text-white placeholder:text-neutral-600 focus:border-red-500/50 text-xs"
                />
              </div>
              ...
              <div className="space-y-2">
                ...
                <Textarea
                  ...
                  rows={5}
                  className="bg-[#0a0a0a] border-white/[0.06] text-white placeholder:text-neutral-600 focus:border-red-500/50 text-xs resize-none"
                />
              </div>

              {/* Email toggle */}
              <div className="flex items-center justify-between py-2 px-3 rounded-lg bg-white/[0.01] border border-white/[0.04]">
                <div className="flex items-center gap-2">
                  <Send className="h-3.5 w-3.5 text-neutral-500" />
                  <span className="text-xs text-neutral-400">Also send as email</span>
                </div>
                <button
                  onClick={() => setMsgSendEmail(!msgSendEmail)}
                  className={`w-10 h-5 rounded-full transition-all relative ${msgSendEmail ? 'bg-red-500' : 'bg-neutral-700'}`}
                >
                  <div className={`absolute top-0.5 w-4 h-4 rounded-full bg-white shadow transition-all ${msgSendEmail ? 'left-5.5' : 'left-0.5'}`} />
                </button>
              </div>
              ...
              <Button
                onClick={async () => {
                  ...
                  if (!msgBroadcast && msgSelectedUsers.length === 0) {
                  ...
                  try {
                    ...
                    if (msgBroadcast) {
                      ...
                      payload.userIds = msgSelectedUsers;
                    }
                    const res = await api.admin.sendMessage(payload);
                    toast.success(`Message sent to ${res.recipients || 0} users`);
                    setMsgSubject('');
                    setMsgBody('');
                    setMsgSelectedUsers([]);
                  } catch (err: any) {
                ...
                disabled={msgSending}
                className="w-full bg-red-600 hover:bg-red-700 text-white gap-2 text-xs font-medium py-2.5"
              >
                {msgSending ? (
                  <Loader2 className="h-3.5 w-3.5 animate-spin" />
                ) : (
                ...
                {msgSending ? 'Sending...' : msgBroadcast ? 'Broadcast to All Users' : `Send to ${msgSelectedUsers.length} User(s)`}
              </Button>
            </CardContent>
          </Card>
        </TabsContent>
          ...
          <Card className="border-white/[0.04] bg-white/[0.01]">
            ...
            <CardContent>
              {auditLoading ? (
                <div className="space-y-2">
                  {[...Array(5)].map((_, i) => (
                    <Skeleton key={i} className="h-10 w-full bg-white/[0.03]" />
                  ))}
                </div>
              ) : auditLogs.length === 0 ? (
                <p className="text-sm text-neutral-500 py-6 text-center">No audit log entries found.</p>
              ) : (
                <>
                  <div className="max-h-96 overflow-y-auto custom-scrollbar">
                    <Table>
                      <TableHeader>
                        <TableRow className="border-white/[0.04] hover:bg-transparent">
                          <TableHead className="text-[10px] text-neutral-500 font-medium">Date</TableHead>
                          <TableHead className="text-[10px] text-neutral-500 font-medium">Admin</TableHead>
                          <TableHead className="text-[10px] text-neutral-500 font-medium">Action</TableHead>
                          <TableHead className="text-[10px] text-neutral-500 font-medium">Target User</TableHead>
                          <TableHead className="text-[10px] text-neutral-500 font-medium">Details</TableHead>
                        </TableRow>
                      </TableHeader>
                      <TableBody>
                        {auditLogs.map((log: any) => {
                          ...
                          return (
                            <TableRow key={log.id} className="border-white/[0.03] hover:bg-white/[0.01]">
                              <TableCell className="text-xs text-neutral-400 py-2.5 whitespace-nowrap">
                                {log.createdAt ? fmtDate(log.createdAt) : 'N/A'}
                              </TableCell>
                              <TableCell className="text-xs text-neutral-300 py-2.5">
                                {log.adminEmail || 'Unknown'}
                              </TableCell>
                              <TableCell className="py-2.5">
                                <Badge className={`${statusBadge(log.action)} border text-[10px]`}>
                                  {log.action}
                                </Badge>
                              </TableCell>
                              <TableCell className="text-xs text-neutral-400 py-2.5">
                                {log.userEmail || log.target || 'N/A'}
                              </TableCell>
                              <TableCell className="text-xs text-neutral-500 py-2.5 max-w-[200px] truncate">
                                {parsedDetails ? (
                                  <span className="text-[10px]">
                                    {parsedDetails.email && `${parsedDetails.email} · `}
                                    {parsedDetails.from !== undefined && `→ ${parsedDetails.to || parsedDetails.amount || ''}`}
                                    {parsedDetails.walletType && `wallet: ${parsedDetails.walletType} ${parsedDetails.amount}`}
                                  </span>
                                ) : (
                                  log.details || '—'
                                )}
                  ...
                  {auditTotal > AUDIT_PAGE_SIZE && (
                    <div className="flex items-center justify-between mt-3 pt-3 border-t border-white/[0.04]">
                      <span className="text-xs text-neutral-500">{auditTotal} entries</span>
                      <div className="flex items-center gap-1">
                        <Button
                          size="sm"
                          variant="outline"
                          className="h-7 text-[10px] px-2 border-white/[0.08] text-neutral-400 hover:text-white hover:bg-white/[0.03]"
                          disabled={auditPage <= 1}
                          onClick={() => { const p = auditPage - 1; setAuditPage(p); fetchAuditLog(p); }}
                        >
                          <ChevronLeft className="h-3 w-3" />
                        </Button>
                        <span className="text-xs text-neutral-500 px-2">{auditPage}</span>
                        <Button
                          size="sm"
                          variant="outline"
                          className="h-7 text-[10px] px-2 border-white/[0.08] text-neutral-400 hover:text-white hover:bg-white/[0.03]"
                          disabled={auditPage * AUDIT_PAGE_SIZE >= auditTotal}
                          onClick={() => { const p = auditPage + 1; setAuditPage(p); fetchAuditLog(p); }}
                        >
                          <ChevronRight className="h-3 w-3" />
                        </Button>
        ...
        <DialogContent className="bg-white/[0.02] border-white/[0.08] text-white sm:max-w-lg max-h-[80vh] overflow-y-auto">
          ...
          {userDetailLoading ? (
            ...
            <div className="space-y-3 py-2">
                ...
                <div className="space-y-2 pt-2 border-t border-white/[0.04]">
                  <div className="flex items-center justify-between">
                    ...
                    <Button
                      size="sm"
                      variant="outline"
                      className="h-6 text-[10px] px-2 border-white/[0.08] text-neutral-400 hover:text-white hover:bg-white/[0.05]"
                      onClick={() => {
                        setEditBalanceUserId(userDetail.id);
                        setEditWalletType(userDetail.wallets?.find((w: any) => w.type === 'live') ? 'live' : 'demo');
                        setEditAmount('');
                      }}
                    >
              ...
              {editBalanceUserId === userDetail?.id && (
                <div className="pt-2 border-t border-white/[0.04] space-y-2">
                  <p className="text-xs text-neutral-500 font-medium">Adjust Balance</p>
                  <div className="flex gap-2">
                    <Select value={editWalletType} onValueChange={setEditWalletType}>
                      <SelectTrigger className="h-8 text-xs bg-black/30 border-white/[0.08]">
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
                        {userDetail.wallets?.map((w: any) => (
                          <SelectItem key={w.type} value={w.type} className="text-xs capitalize">
                            {w.type} (${fmt(w.balance || 0)})
                          </SelectItem>
                        ))}
                      </SelectContent>
                    </Select>
                    <Input
                      ...
                      className="h-8 text-xs bg-black/30 border-white/[0.08] placeholder:text-neutral-600"
                      value={editAmount}
                    ...
                    <Button
                      ...
                      disabled={editBalanceLoading || !editAmount}
                      onClick={handleAdjustBalance}
                    >
                      {editBalanceLoading ? <Loader2 className="h-3 w-3 animate-spin" /> : 'Apply'}
                    </Button>
                  </div>
                  <p className="text-[10px] text-neutral-600">Use positive to add funds, negative to deduct. Creates an audit transaction record.</p>
                </div>
              )}
            </div>
          ) : (
