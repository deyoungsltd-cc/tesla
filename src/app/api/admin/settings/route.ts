...
export async function GET() {
  try {
    ...
    return apiResponse({
      ...
      slideshowModels: settings.slideshowModels,
    });
  } catch (error) {
...
export async function PUT(request: NextRequest) {
  ...
  try {
    ...
    const { aboutPhotoUrl, elonPhotoUrl, slideshowModels } = body;
    ...
    if (Array.isArray(slideshowModels)) {
      updateData.slideshowModels = slideshowModels;
    }
    if (Object.keys(updateData).length === 0) {
      return apiError('No valid fields to update', 'MISSING_DATA', 400);
    }
    ...
    return apiError('Failed to update settings', 'INTERNAL_ERROR', 500);
  }
}
