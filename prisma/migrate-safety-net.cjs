...
const DDL_STATEMENTS = [
  ...
  // ENUM TYPES
  // ══════════════════════════════════════════════════════════
  // NOTE: PostgreSQL does NOT support IF NOT EXISTS for CREATE TYPE ... AS ENUM.
  // We use plain CREATE TYPE and rely on the per-statement try/catch to
  // ignore duplicate_object (42P07) errors on subsequent startups.
  ...
  `CREATE TYPE "WalletType" AS ENUM ('demo','live')`,
  `CREATE TYPE "TransactionType" AS ENUM ('deposit','withdrawal','investment','investment_return','referral_bonus','fee','promo_credit','balance_adjustment')`,
  `CREATE TYPE "TransactionStatus" AS ENUM ('pending','completed','failed','reversed')`,
  `CREATE TYPE "InvestmentStatus" AS ENUM ('active','completed','failed','cancelled')`,
  `CREATE TYPE "DepositMethod" AS ENUM ('crypto','gift_card')`,
  `CREATE TYPE "CryptoCurrency" AS ENUM ('BTC','ETH','USDT')`,
  `CREATE TYPE "DepositStatus" AS ENUM ('pending','pending_verification','confirmed','rejected','expired')`,
  `CREATE TYPE "WithdrawalStatus" AS ENUM ('pending','processing','completed','rejected','failed')`,
  `CREATE TYPE "WithdrawalDestinationType" AS ENUM ('crypto','bank')`,
  `CREATE TYPE "ReferralCommissionType" AS ENUM ('direct','binary')`,
  `CREATE TYPE "ReferralCommissionStatus" AS ENUM ('pending','paid','reversed')`,
  ...
  `CREATE TYPE "AdminRoleName" AS ENUM ('SUPER_ADMIN','ADMIN','COMPLIANCE','SUPPORT')`,
  `CREATE TYPE "NotificationType" AS ENUM ('deposit_confirmed','deposit_rejected','withdrawal_processed','withdrawal_rejected','investment_activated','investment_completed','investment_return_credited','referral_earned','kyc_submitted','kyc_approved','kyc_rejected','kyc_reminder','security_login_detected','security_password_changed','ticket_response','ticket_resolved','system_maintenance','system_announcement','vehicle_order_placed','vehicle_order_confirmed','vehicle_order_shipped','vehicle_order_delivered','vehicle_order_cancelled','custom')`,
  `CREATE TYPE "OtpType" AS ENUM ('email_verification','password_reset','two_factor_setup','two_factor_disable','withdrawal_confirm')`,
  `CREATE TYPE "GiftCardStatus" AS ENUM ('pending','verified','rejected','expired')`,
  `CREATE TYPE "EmailStatus" AS ENUM ('sent','delivered','failed','bounced')`,
  `CREATE TYPE "PayoutStatus" AS ENUM ('pending','processed','failed')`,
  `CREATE TYPE "UserMode" AS ENUM ('demo','live')`,
  `CREATE TYPE "DurationUnit" AS ENUM ('hours','days')`,
  `CREATE TYPE "PromoDiscountType" AS ENUM ('percentage','fixed')`,
  `CREATE TYPE "PromoCodeStatus" AS ENUM ('active','inactive','expired','fully_redeemed')`,
  `CREATE TYPE "VehicleColor" AS ENUM ('pearl_white','solid_black','midnight_silver','deep_blue','red_multi_coat','ultra_red','quick_silver','blue_multi_coat')`,
  `CREATE TYPE "VehicleOrderStatus" AS ENUM ('pending','confirmed','in_production','shipped','delivered','cancelled')`,
