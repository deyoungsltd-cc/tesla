...
export default function RegisterPage() {
  ...
  const validate = () => {
    ...
    if (referralCode.trim() && referralCode.trim().length !== 8) {
      ...
      <div className="hidden lg:flex lg:w-1/2 relative items-center justify-center border-r border-tesla-gray-800">
        ...
        <div className="relative z-10 flex flex-col items-center justify-center px-16">
          ...
          <p className="mt-8 text-tesla-gray-500 text-xs tracking-wider max-w-xs text-center leading-relaxed">
            Join thousands of investors earning daily returns on our
            premium managed investment platform.
          </p>
        </div>
      ...
      <div className="flex-1 flex items-center justify-center px-6 py-12 sm:px-12 lg:px-20">
        <div className="w-full max-w-[420px]">
          ...
          <p className="mt-2 text-tesla-gray-300 text-sm">
            Start your investment journey with TeslaPrimeCapital
          </p>
          ...
          <form onSubmit={handleSubmit} className="mt-8 space-y-5">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                />
                {fieldErrors.firstName && (
                  <p className="mt-1.5 text-xs text-tesla-red">{fieldErrors.firstName}</p>
                )}
              </div>
              <div>
                <label
                ...
                />
                {fieldErrors.lastName && (
                  <p className="mt-1.5 text-xs text-tesla-red">{fieldErrors.lastName}</p>
                )}
              </div>
            </div>
            ...
            <div>
              ...
              {fieldErrors.confirmPassword && (
                <p className="mt-1.5 text-xs text-tesla-red">{fieldErrors.confirmPassword}</p>
              )}
            </div>
            ...
            <div>
              ...
              >
                Referral Code{' '}
                <span className="normal-case tracking-normal text-tesla-gray-600">(optional)</span>
              </label>
              <input
                id="referralCode"
                type="text"
                ...
                maxLength={8}
                className={`w-full h-12 px-4 bg-tesla-gray-900 text-tesla-white placeholder-tesla-gray-500 border rounded-none outline-none transition-colors duration-200 text-sm uppercase ${
              ...
              />
              {fieldErrors.referralCode && (
                <p className="mt-1.5 text-xs text-tesla-red">{fieldErrors.referralCode}</p>
              )}
            </div>
            ...
            <div>
              <label className="flex items-start gap-3 cursor-pointer group">
                <button
                  ...
                  className="mt-0.5 w-5 h-5 border flex items-center justify-center transition-colors shrink-0"
                  aria-label="Agree to terms"
                >
                  <div
                    className={`w-full h-full flex items-center justify-center ${
                      agreedToTerms
                        ? 'bg-tesla-red border-tesla-red'
                        : 'border-tesla-gray-600 bg-tesla-gray-900 group-hover:border-tesla-gray-500'
                    }`}
                  >
                    {agreedToTerms && (
                      <svg
                ...
                <span className="text-xs text-tesla-gray-400 leading-relaxed">
                  I agree to the{' '}
                  <span className="text-tesla-gray-300 hover:text-tesla-white transition-colors cursor-pointer">
                    Terms of Service
                  </span>{' '}
                  and{' '}
                  <span className="text-tesla-gray-300 hover:text-tesla-white transition-colors cursor-pointer">
              ...
              {fieldErrors.terms && (
                <p className="mt-1.5 text-xs text-tesla-red ml-8">{fieldErrors.terms}</p>
              )}
            </div>
