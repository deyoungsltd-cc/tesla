...
async function seed() {
  console.log('🌱 Seeding TeslaPrime database...');

  // Create Super Admin
  const adminPassword = await hashPassword('Admin@123');
  const superAdmin = await prisma.user.upsert({
    where: { email: 'admin@teslaprimeital.com' },
    update: {},
    create: {
      email: 'admin@teslaprimeital.com',
  ...
  console.log('📧 Admin: admin@teslaprimeital.com / Admin@123');
