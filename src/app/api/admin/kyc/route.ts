...
async function updateHandler(request: NextRequest) {
  try {
    ...
    const verification = await db.kYCVerification.findUnique({ where: { id: verificationId } });
      ...
      const result = await db.$transaction(async (tx) => {
        ...
        await tx.notification.create({
          data: {
            ...
            type: 'kyc_approved',
            title: 'KYC Approved',
            message: `Your KYC verification (Level ${verification.level}) has been approved. Your account level is now ${nextLevel}.`,
          },
        });
      ...
      const result = await db.$transaction(async (tx) => {
        ...
        await tx.notification.create({
          data: {
            ...
            type: 'kyc_rejected',
            title: 'KYC Rejected',
            message: `Your KYC verification was rejected. Reason: ${reason || 'Not specified'}. Please resubmit with correct documents.`,
          },
        });
