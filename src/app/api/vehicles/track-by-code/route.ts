...
// GET - Track by order number (user's own orders only)
async function handler(request: NextRequest, _context: any, user: any) {
  try {
    ...
    if (!orderNumber) {
      return apiError('Order number is required', 'MISSING_ORDER_NUMBER', 400);
    }
    ...
    if (!order) {
      return apiError('Order not found. Check the order number and try again.', 'ORDER_NOT_FOUND', 404);
    }
    ...
    let timeline = trackingInfo.timeline || [];
    ...
    const estimatedDates: Record<string, string> = {
      ...
      delivered: trackingInfo.estimatedDelivery || new Date(orderDate.getTime() + 45 * 86400000).toISOString(),
    };

    const routeStages = [
      { key: 'pending', label: 'Order Placed', location: 'Online', direction: 'Processing' },
      { key: 'confirmed', label: 'Confirmed', location: 'Tesla HQ, Austin TX', direction: 'Processing' },
