...
export default function TeslaLogo({ className = 'h-8', variant = 'full' }: TeslaLogoProps) {
  ...
  // Full variant: icon + "TESLA PRIME" text (for login, signup, hero)
  if (variant === 'wordmark') {
    return (
      <div className={`flex items-center gap-2 ${className}`}>
        ...
        <div className="flex flex-col leading-none">
          ...
          <span className="text-gray-400 font-semibold tracking-[0.15em] text-[0.65em] mt-0.5" style={{ fontFamily: 'Arial, Helvetica, sans-serif' }}>PRIME</span>
        </div>
      </div>
    );
  }
